...
there is a pod deployed via coredns , and if we can reach with ip and not hostname that means there is something wrong with this 
...
coredns is the deployment name , kube-dns is the service name for the dns 
...
So now when i make a service the dns is already added to the dns system of the cluster and then now when it comes to the need for this is because the ip of the pod is temporary hence why i would need an internal dns system 
