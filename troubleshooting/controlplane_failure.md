...
we can check teh logs of the exited container as well.
there could be a chance there is an error in the kubeconig file as well.
...
So now say for example we have a deployment with 2 pods , but then i delete one and then it doesnt get recreated , but the deployment shows 2/2 , this is a fault with the controller as this handles all the controller manager of the cluster 